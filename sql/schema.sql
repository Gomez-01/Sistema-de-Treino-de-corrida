...
CREATE TABLE alunos (
    ...
    nivel_condicionamento VARCHAR(30) CHECK (nivel_condicionamento IN ('iniciante', 'intermediario', 'avancado')),
...
CREATE TABLE planos (
    ...
    tipo_periodo VARCHAR(20) CHECK (tipo_periodo IN ('mensal', 'trimestral', 'semestral')),
    tipo_acomp VARCHAR(20) CHECK (tipo_acomp IN ('individual', 'grupo')),
...
CREATE TABLE contratos (
    ...
    aluno INTEGER NOT NULL REFERENCES alunos(id_aluno),
    id_plano INTEGER NOT NULL REFERENCES planos(id_plano),
    data_inicio DATE NOT NULL,
    data_fim DATE NOT NULL,
    status VARCHAR(20) CHECK (status IN ('ativo', 'encerrado', 'cancelado')),
    criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
